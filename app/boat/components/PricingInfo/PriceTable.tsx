      ...
      <div className="hidden md:block overflow-x-auto my-6">
        <table className="w-full">
          <thead className="bg-slate-100">
            <tr>
              <th className="px-4 py-3 text-left text-sm font-semibold text-[#17384A]">
                Season
              </th>
              <th className="px-4 py-3 text-left text-sm font-semibold text-[#17384A]">
                Months
          ...
          <tbody className="divide-y divide-slate-200">
            {pricingData.seasonalPricing.map((item) => (
              <tr key={item.season}>
                <td className="px-4 py-3 text-sm font-medium text-[#17384A]">
                  {item.season}
                </td>
                <td className="px-4 py-3 text-sm text-slate-600">
                  {item.months}
                </td>
                <td className="px-4 py-3 text-sm font-semibold text-[#c79432]">
        ...
        {pricingData.seasonalPricing.map((item) => (
          <div key={item.season} className="bg-slate-50 p-4 shadow-sm">
            <div className="flex justify-between items-start mb-3">
              <div>
                <h3 className="font-bold text-[#17384A] text-lg">
                  {item.season}
                </h3>
            ...
            <div className="space-y-2 pt-2 border-t border-slate-200">
              <div className="flex justify-between items-center">
                ...
                <span className="font-semibold text-[#c79432]">
                  {item.priceWithoutSkipper} €/day
                </span>
              </div>
              <div className="flex justify-between items-center">
