                ...
                >
                    {items.map((item, index) => (
                        <article
                            ...
                            style={{ width: `calc((100% - ${gap * (safeCardsPerView - 1)}px) / ${safeCardsPerView})` }}
                        >
                            <img src={item.image} alt={item.title} className="h-52 md:h-70 w-full object-cover" />
                            <div className="p-4 md:p-5">
                                <h3 className="text-lg md:text-xl font-bold text-[#17384A]">{item.title}</h3>
                ...
                <div className="mt-4 flex items-center justify-center gap-2">
                    {Array.from({ length: totalPositions }).map((_, index) => (
                        <button
                            ...
                            className={`h-2.5 w-2.5 rounded-full transition ${index === currentIndex ? "bg-[#c79432] w-6" : "bg-gray-400"
                                }`}
